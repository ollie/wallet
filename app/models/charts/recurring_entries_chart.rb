module Charts
  class RecurringEntriesChart
    attr_accessor :date, :months_data
    ...
    def data
      recurring_entries =
        ...
        .where(enabled: true)
        .where(Sequel.lit('starts_on <= ?', Date.new(date.year, 12, 31)))
      ...
      recurring_entries.each do |recurring_entry|
        ...
        loop do
          ...
          break if month_date.year != date.year
    ...
    def transform_months_data
      {}.tap do |hash|
        months_data.each do |date, data|
          hash[I18n.l(date, format: '%B')] = data
        end
      end
    end
  end
